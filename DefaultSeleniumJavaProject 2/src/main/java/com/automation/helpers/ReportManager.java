...
public class ReportManager {
    ...
    public void closeTestReport() {
        if (!reportFileClosed) {
            logger.info("closeing Test Report file and test");
            if (!testClosed) {
                extent.endTest(test);
                testClosed = true;
            }
    ...
    public void startTest(String suiteName, String methodeName, String category) {
        ...
        test.assignAuthor("Ehud Kon");
        test.assignCategory(category);
        testClosed = false;
        clickFailed = false;
        logger.info("Test " + test.getTest().getName() + " were started");
    }
    ...
    public void closeTest() {
        if (!testClosed) {
            extent.endTest(test);
            testClosed = true;
            logger.info("Test " + test.getTest().getName() + " were closed");
        }
    }
    ...
    public void reportException(Exception e, boolean throwException) {
        ...
        logger.info("reportException Thread: " + Thread.currentThread().getName());
